...
//        scenario.addOperation(new FeaturesExtractionOperation(stageParameters, ExecParams._initialExecParameters, ExecParams._initialExecParameters._waveFilesTestFileList, false, 1));
        ...
        for (String s : fileCorrespondanceMap.keySet()) {
            HelperFile.copyFile(resultsDir + HelperFile.getShortFileNameWithoutExt(fileCorrespondanceMap.get(s)) + Extensions.LABEL_EXT, outLabelsDir + HelperFile.getShortFileNameWithoutExt(s) + Extensions.LABEL_EXT);
            HelperFile.copyFile(resultsDirLM + HelperFile.getShortFileNameWithoutExt(fileCorrespondanceMap.get(s)) + Extensions.LABEL_EXT, outLabelsDirLM + HelperFile.getShortFileNameWithoutExt(s) + Extensions.LABEL_EXT);
        }
