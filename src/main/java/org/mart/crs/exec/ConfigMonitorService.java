...
public class ConfigMonitorService {
    ...
    public static int maxSlots = Settings.NumberOfParallelThreadsForConfigListenerService;
    ...
    public ConfigMonitorService(String monitorDirFilePath) {
        ...
        poolExecutor = new CRSThreadPoolExecutor(maxSlots * Settings.numberOfFolds);
        ...
        while (true) {
            ...
            File[] files = dirToMonitor.listFiles(new ExtensionFileFilter(Extensions.CFG_EXT, false));
            if (files == null || files.length == 0) {
                continue;
            }
            for (File file : files) {
                if (getNumberOfSlots() + 1 <= maxSlots) {
                    executeConfig(file);
                }
            }
        }
            ...
            poolExecutor.runTask(new Runnable() {
                public void run() {
                    try {
                        ...
                        String cmd = String.format("java -Xmx%s -DoutPathExternal=%s%s -Dfold=%d -jar ./%s/mart-mir-1.0-SNAPSHOT.jar -c %s", finalMemorySize, outPathForThisConfig, specialSeparator, finalI, newLibsDirName, destConfig);
                        Helper.execCmd(cmd);
                    } catch (Exception e) {
                        logger.error(Helper.getStackTrace(e));
                    }
