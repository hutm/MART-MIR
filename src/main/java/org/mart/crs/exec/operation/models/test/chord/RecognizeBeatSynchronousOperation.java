...
public class RecognizeBeatSynchronousOperation extends RecognizeOperation {
    ...
    public void operate() {
        ...
        command = command + String.format(" -H %s/%s/%s -T 1 -S %s -i %s -n %d %d -w %s -p %5.2f %s %s",
                trainedModelsDir, hmmFolder + "_" + gaussianNumber, macros, featureFileListTest, outFilePath, ChordSmartLatticeDomain.maxNumberOfHypos, ChordSmartLatticeDomain.maxNumberOfHypos,
        ...
        for (ChordStructure song : songList) {
            ...
            aLattice.storeInFile(String.format("%s/%s%s", latticeDir, song.getSongName(), Extensions.LATTICE_EXT));
        }

        //Creating list of lattices
        String fileList = String.format("%s/latticeList_%d_%3.2f.txt", tempDirPath, gaussianNumber, penalty);
        HelperFile.createFileList(latticeDir, fileList, new ExtensionFileFilter(Extensions.LATTICE_EXT), false);
...
//            if (IS_FACTORED_LM) {        //TODO: refactoring
//                command = "lattice-tool" + EXECUTABLE_EXTENSION + " -debug 0 -in-lattice " + inLattice + " -out-lattice " + outLattice + " -read-htk -write-htk -factored -lm " + lmDir + File.separator + LMSpecFactoredFileName + " -htk-logbase 2.71828 -no-nulls -no-htk-nulls -order " + execParams.latticeRescoringOrder;
//            } else {
                command =  "lattice-tool" + EXECUTABLE_EXTENSION + " -debug 0 -in-lattice " + inLattice + " -out-lattice " + outLattice + " -read-htk -write-htk -lm " + lmDir + File.separator + LMModelStandardVersion + " -htk-logbase 2.71828 -no-nulls -no-htk-nulls -order " + execParams.latticeRescoringOrder;
//            }
            ...
            for (int j = 0; j < execParams._acWeights.length; j++) {
                for (int k = 0; k < execParams._wips.length; k++) {
                    ...
                    try {
                        ...
                        command = "lattice-tool" + EXECUTABLE_EXTENSION + " -debug 0 -in-lattice-list " + fileList + " -read-htk -htk-lmscale " + lmWeight + " -htk-wdpenalty " + wip + " -htk-acscale " + acWeight + " -nbest-viterbi -viterbi-decode -output-ctm";
