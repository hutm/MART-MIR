   ...
   protected  float[] goodIndecesWeights = new float[]{1, 1, 1, 0.3f, 0.3f, 0.3f, 0.3f, 0.3f, 0.3f, 0.3f, 0.3f};
    ...
    public void operate() {
        ...
        for (File aFile : fileList) {
            ...
            SpectrumImpl spectrum1 = CoreElementsInitializer.initializeSpectrumWithExecParamsData(SpectrogramType.FFT_BASED, reader.getSamples(), reader.getSampleRate(), _initialExecParameters);
            ReassignedSpectrum spectrum2 = (ReassignedSpectrum)CoreElementsInitializer.initializeSpectrumWithExecParamsData(SpectrogramType.STANDARD, reader.getSamples(), reader.getSampleRate(), _initialExecParameters);
            ReassignedSpectrum spectrum3 = (ReassignedSpectrum)CoreElementsInitializer.initializeSpectrumWithExecParamsData(SpectrogramType.HARMONIC, reader.getSamples(), reader.getSampleRate(), _initialExecParameters);
...
//        data = HelperArrays.normalizeVector(data);
    ...
    protected float[] getPCPPart(float[] pcpFull) {
        ...
        for (int i = _initialExecParameters.startMidiNote; i <= _initialExecParameters.endMidiNote; i++) {
            out[i - _initialExecParameters.startMidiNote] = pcpFull[i - Configuration.START_NOTE_FOR_PCP_UNWRAPPED];
        }
        return out;
    }
