...
public class TempoExtractionOperation extends Operation {
    ...
    public void operate() {
        ...
        File[] files = extractedDir.listFiles(new ExtensionFileFilter(Extensions.BEAT_EXT));
        ...
        for(File songName:files){
            ...
            extractedTempos.put(songName.getName().replaceAll("signal", "").replaceAll(Extensions.BEAT_EXT, ""), tempo);
        }

        //save in the txt file
        String outFileName = String.format("%s%s", extractedDir, Extensions.TXT_EXT);
