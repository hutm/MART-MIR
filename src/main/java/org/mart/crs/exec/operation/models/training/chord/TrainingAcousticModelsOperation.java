    ...
    public void operate() {
        ...
        for (int g = 1; g <= execParams.gaussianNumber; g *= 2) {
            ...
            if (g > 1) {
                ...
                command = String.format("HHEd%s -H %s -H %s -M %s %s %s", EXECUTABLE_EXTENSION,
            ...
            for (int i = startHmmNumber; i < numberOfIterationsHERest; i++) {
                command = String.format("HERest%s -t 250.0 -C %s -I %s -S %s -H %s -H %s -M %s %s", EXECUTABLE_EXTENSION,
