...
public class RecognizeBeatOperation extends RecognizeOnsetOperation {
    ...
    protected void createGrammarFile() {
        try {
            ...
            for (int beatLength = BeatOperationDomain.minNimberOfFramesForBeatSegment; beatLength < BeatOperationDomain.maxNimberOfFramesForBeatSegment; beatLength++) {
    ...
    protected void hvite(String trainedModelsDir, String hmmFolder, float penalty, String decodedOutPath) {
        String command = String.format("HVite%s -t 250.0 -C %s ", EXECUTABLE_EXTENSION, configPath);
