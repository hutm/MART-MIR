...
import static org.mart.crs.exec.operation.domain.BeatOperationDomain.maxNimberOfFramesForBeatSegment;
import static org.mart.crs.exec.operation.domain.BeatOperationDomain.minNimberOfFramesForBeatSegment;
...
public class FeaturesManagerBeat extends FeaturesManagerChord {
    ...
    public FeaturesManagerBeat(String songFilePath, String outDirPath, boolean isForTraining, ExecParams execParams) {
        ...
        labelsSource = new LabelsSource(Settings.labelsGroundTruthDir, true, "gt", Extensions.BEAT_EXTENSIONS);
    }
...
//        lblFilePath = HelperFile.getPathForFileWithTheSameName(songFilePath, Settings.labelsGroundTruthDir, Extensions.BEAT_EXT);
    ...
    protected void extractBeatsInOneSegment(String lblFilePath, float refFrequency, String dirName) {
        ...
        for (int i = 0; i < segments.size() - 1; i++) {    //The last beat in each song is not included in training set
            ...
            String filename = String.format("%5.3f_%5.3f_%s%d%s", startTime, endTime, curSegment.toString(), numberOframes, Extensions.CHROMA_EXT);
