...
public class UnwrappedReasWeighted extends UnwrapReas {
    ...
    protected float[][] cutPCPUnwrapped(PCP pcp) {
        ...
        for (int frame = 0; frame < pcp.getPCPUnwrapped().length; frame++) {
            for (int octaveStartNote = startMidiNote - Configuration.START_NOTE_FOR_PCP_UNWRAPPED; octaveStartNote < endMidiNote - Configuration.START_NOTE_FOR_PCP_UNWRAPPED; octaveStartNote += Configuration.NUMBER_OF_SEMITONES_IN_OCTAVE) {
                ...
                if (!isToWrapFinally) {
                    for (int i = 0; i < 12; i++) {
                        outPCP[frame][octaveStartNote + i - (startMidiNote - Configuration.START_NOTE_FOR_PCP_UNWRAPPED)] = summedOctaves[i];
                    }
                } else{
                    for (int i = 0; i < 12; i++) {
