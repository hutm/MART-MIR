 ...
 * @author: Hut
 */
public class BassJNI extends SpectrumBased {
    @Override
    protected PCP producePCP(float[] samples, double refFrequency) {
        JNIChromaExtractor extractor = new JNIChromaExtractor();
        float[][] pcpData = extractor.getFeaturesInMartFormatBass(this.audioReader.getFilePath(), execParams.samplingRate, execParams.windowLength, execParams.overlapping);
