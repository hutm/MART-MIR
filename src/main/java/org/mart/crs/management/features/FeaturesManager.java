...
public class FeaturesManager {
    ...
    protected void processSong(float refFrequency, String dirName) {
        if (isForTraining) {
            ...
            if (ChordOperationDomain.isBeatSynchronousDecoding) {
    ...
    public void exctractForTraining(float refFrequency, String dirName) {
        String lblFilePath;
        if (!Settings.isMIREX) {
            lblFilePath = HelperFile.getPathForFileWithTheSameName(songFilePath, Settings.labelsGroundTruthDir, Extensions.LABEL_EXT);
        } else {
