...
public class UnwrapReas extends TrebleReas {
    ...
    protected float[][] cutPCPUnwrapped(PCP pcp) {
        ...
        for (int frame = 0; frame < pcp.getPCPUnwrapped().length; frame++) {
            for (int i = execParams.startMidiNote - Configuration.START_NOTE_FOR_PCP_UNWRAPPED; i < execParams.endMidiNote - Configuration.START_NOTE_FOR_PCP_UNWRAPPED; i++) {
                outPCP[frame][i - (execParams.startMidiNote - Configuration.START_NOTE_FOR_PCP_UNWRAPPED)] = pcp.getPCPUnwrapped()[frame][i];
            }
        }
        return outPCP;
    }
    ...
    public float[][] rotateFeatures(float[][] feature, Root chordNameFrom) {
        ...
        for (int startNote = 0; startNote < feature[0].length; startNote += Configuration.NUMBER_OF_SEMITONES_IN_OCTAVE) {
            float[][] octavePCP = new float[feature.length][Configuration.NUMBER_OF_SEMITONES_IN_OCTAVE];
            for (int i = 0; i < feature.length; i++) {
                octavePCP[i]=Arrays.copyOfRange(feature[i], startNote, startNote + Configuration.NUMBER_OF_SEMITONES_IN_OCTAVE);
            }
            octavePCP  = PCP.rotatePCP(octavePCP, 1, chordNameFrom, Root.C);
            for (int i = 0; i < feature.length; i++) {
                System.arraycopy(octavePCP[i], 0, output[i], startNote,  Configuration.NUMBER_OF_SEMITONES_IN_OCTAVE);
            }
        }
        return output;
    }
