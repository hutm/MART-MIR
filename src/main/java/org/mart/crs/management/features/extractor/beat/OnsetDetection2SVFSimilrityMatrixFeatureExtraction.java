...
import org.mart.crs.core.similarity.SimilarityMatrix;
...
/**
 * @version 1.0 3/16/11 8:47 PM
 * @author: Hut
 */
public class OnsetDetection2SVFSimilrityMatrixFeatureExtraction extends OnsetDetectionChordChangeFeatureExtractor {

    @Override
    protected void extractGlobalFeatures(double refFrequency) {
        ReassignedSpectrumPercussivePart spectrumPercussive = new ReassignedSpectrumPercussivePart(audioReader, execParams.windowLengthOnsetDetection, execParams.windowType, execParams.overlappingOnsetDetection, execParams.beatReasPercussivePartThreshold);
        ...
        ReassignedSpectrumHarmonicPart spectrumHarmonic = new ReassignedSpectrumHarmonicPart(audioReader, execParams.windowLengthSVFChroma, execParams.windowType, execParams.overlappingSVFChroma, execParams.beatReasHarmonicPartThreshold);


        PCP pcp = CoreElementsInitializer.initializePCPWithExecParamsData(PCP.BASIC_ALG, refFrequency, execParams);
        pcp.initReassignedSpectrum(spectrumHarmonic);
        ...
        for (int i = 0; i < length; i++) {
            ...
            out[i][3] = onsetDetectionFromSelfSimilarityMatrix[i];
        }
